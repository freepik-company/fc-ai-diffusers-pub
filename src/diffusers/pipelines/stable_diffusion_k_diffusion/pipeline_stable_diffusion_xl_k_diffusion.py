...
from ...models import AutoencoderKL, UNet2DConditionModel, ImageProjection
...
from ...pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl import rescale_noise_cfg
...
class StableDiffusionXLKDiffusionPipeline(StableDiffusionXLPipeline):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion XL and [`K-diffusion`] library.

    This model inherits from [`StableDiffusionXLPipeline`]. Check the superclass documentation for the generic methods the library implements for all the pipelines (such as downloading or saving, running on a particular device, etc.)
    
    Besides, the model is based on [`examples.community.sd_text2img_k_diffusion.py`], so thanks for their amazing contribution
    ...
    def set_scheduler(self, scheduler_type: str):
        ...
        sampling = getattr(library, "sampling")  # noqa: B009
        try:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        def model_fn(
            ...
            # expand the latents and timestep if we are doing classifier free guidance
        ...
        if "noise_sampler" in inspect.signature(self.sampler).parameters:
            ...
            noise_sampler = BrownianTreeNoiseSampler(
                latents, min_sigma, max_sigma, generator.initial_seed()
            )
        ...
        latents = self.sampler(
            partial(
                model_fn,
                k_diffusion_model=self.k_diffusion_model,
                do_classifier_free_guidance=self.do_classifier_free_guidance,
